...
# Configurazione password statica per la registrazione
REGISTRATION_PASSWORD = "MioGestionale2024!"  # Cambia questa password
...
def get_cookie(name):
    ...
    cookie_string = components.html(f"""
    <script>
...
def show_auth_page(auth):
            ...
            if submit_login:
                if username and password:
                    ...
                    if user_data:
                        ...
                        st.success(f"Benvenuto, {user_data['nome']} {user_data['cognome']}!")
                        st.rerun()
                    else:
    ...
    with tab2:
        ...
        with st.form("register_form"):
            reg_password_access = st.text_input("Password di accesso alla registrazione", type="password")
            
            st.divider()
            
            reg_username = st.text_input("Username desiderato")
            ...
            if submit_register:
                # Verifica password di accesso alla registrazione
                if reg_password_access != REGISTRATION_PASSWORD:
                    st.error("Password di accesso alla registrazione non corretta!")
                elif not all([reg_username, reg_nome, reg_cognome, reg_password, reg_password_confirm]):
                    ...
                    if auth.create_user(reg_username, reg_nome, reg_cognome, reg_password):
...
def show_dashboard(auth):
    ...
    def get_reservations_by_filter(date_filter=None, status_filter=None, name_filter=None, equipment_filter=None):
    ...
    if st.session_state.logged_in and "user_data" in st.session_state:
        ...
        if st.session_state.current_page == "home":
            ...
            recent_rentals = sorted(st.session_state.reservations, 
                                key=lambda x: x.get('created_at', ''), reverse=True)[:30]
    ...
    # Applica filtri e ottieni prenotazioni filtrate
                ...
                for i, rental in enumerate(filtered_reservations):
                    ...
                    with col1:
                        if not rental.get('completed', False):
                            if st.button("✅ Completa", key=f"complete_{rental['id']}", use_container_width=True):
                                if update_reservation_status(rental['id'], True):
                                    st.success("Noleggio completato!")
                                    refresh_reservations()
                                    st.rerun()
                        else:
                            if st.button("↩️ Riattiva", key=f"reactivate_{rental['id']}", use_container_width=True):
                    ...
                    with col2:
                        if st.button("💳 Toggle Deposito", key=f"deposit_{rental['id']}", use_container_width=True):
                            new_deposit_status = not rental.get('deposit_paid', False)
                            if update_reservation_status(rental['id'], rental.get('completed', False), new_deposit_status):
                                st.success(f"Deposito {'pagato' if new_deposit_status else 'non pagato'}!")
                                refresh_reservations()
                                st.rerun()

                    with col3:
                        if st.button("🗑️ Elimina", key=f"delete_{rental['id']}", use_container_width=True):
            ...
            total_ombrelloni = sum([r.get('ombrellone', 0) for r in st.session_state.reservations])
            total_sdraio = sum([r.get('sdraio', 0) for r in st.session_state.reservations])
            total_lettini = sum([r.get('lettino', 0) for r in st.session_state.reservations])
            total_regista = sum([r.get('regista', 0) for r in st.session_state.reservations])
            
            # Ricavi
            total_revenue = sum([r.get('price', 0) for r in st.session_state.reservations])
            paid_deposits = len([r for r in st.session_state.reservations if r.get('deposit_paid', False)])
            ...
            st.info(f"**Nome Utente:** {st.session_state.user_data['nome']} {st.session_state.user_data['cognome']}")
            ...
            with col2:
                ...
                if uploaded_file and st.button("📤 Importa Backup", use_container_width=True):
                    try:
                        import json
